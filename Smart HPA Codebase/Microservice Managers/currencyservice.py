...
#***************************************************** This file outlines the algorithm for Microservice Manager responsible for currencyservice microservice **********************************************
...
def Monitor ():
    ...
    while Available_Replicas != Operational_Replicas:               # To accomodate pod initialization time
        ...
        Available_Replicas = "kubectl get deployment currencyservice -o=jsonpath='{.status.availableReplicas}'"
        ...
        Replicas_CPU_usage = 'kubectl top pods -l app=currencyservice'                                     # Monitoring CPU Usage per replica
    ...
    Desired_Replicas = "kubectl get deployment currencyservice -o=jsonpath='{.spec.replicas}'"
    ...
    cpu_request = "kubectl get deployment currencyservice -o=jsonpath='{.spec.template.spec.containers[0].resources.requests.cpu}'"
...
def currencyservice(Test_Time):
    ...
    currencyservice_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica]

    
    #********************************************************************** Storing Data for each iteration in the Knowledge Base ********************************************************************************

    workbook = load_workbook('./Knowledge Base/currencyservice.xlsx')
    ...
    workbook.save('./Knowledge Base/currencyservice.xlsx')



    return currencyservice_data
