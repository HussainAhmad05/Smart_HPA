...
#***************************************************** This file outlines the algorithm for Microservice Manager responsible for productcatalogservice microservice **********************************************
...
def Monitor ():
    ...
    while Available_Replicas != Operational_Replicas:               # To accomodate pod initialization time
        ...
        Available_Replicas = "kubectl get deployment productcatalogservice -o=jsonpath='{.status.availableReplicas}'"
        ...
        Replicas_CPU_usage = 'kubectl top pods -l app=productcatalogservice'                                     # Monitoring CPU Usage per replica
    ...
    Desired_Replicas = "kubectl get deployment productcatalogservice -o=jsonpath='{.spec.replicas}'"
    ...
    cpu_request = "kubectl get deployment productcatalogservice -o=jsonpath='{.spec.template.spec.containers[0].resources.requests.cpu}'"
...
def productcatalogservice(Test_Time):
    ...
    productcatalogservice_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica]

    
    #********************************************************************** Storing Data for each iteration in the Knowledge Base ********************************************************************************

    workbook = load_workbook('./Knowledge Base/productcatalogservice.xlsx')
    ...
    workbook.save('./Knowledge Base/productcatalogservice.xlsx')



    return productcatalogservice_data
