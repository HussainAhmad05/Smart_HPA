...
#***************************************************** This file outlines the algorithm for Microservice Manager responsible for frontend microservice **********************************************
...
def Monitor ():
    ...
    while Available_Replicas != Operational_Replicas:               # To accomodate pod initialization time
        ...
        Available_Replicas = "kubectl get deployment frontend -o=jsonpath='{.status.availableReplicas}'"
        ...
        Replicas_CPU_usage = 'kubectl top pods -l app=frontend'                                     # Monitoring CPU Usage per replica
    ...
    Desired_Replicas = "kubectl get deployment frontend -o=jsonpath='{.spec.replicas}'"
    ...
    cpu_request = "kubectl get deployment frontend -o=jsonpath='{.spec.template.spec.containers[0].resources.requests.cpu}'"
...
def frontend(Test_Time):
    ...
    workbook = load_workbook('./Knowledge Base/frontend.xlsx')
    ...
    workbook.save('./Knowledge Base/frontend.xlsx')
