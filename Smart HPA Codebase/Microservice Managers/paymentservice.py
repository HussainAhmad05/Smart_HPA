...
#***************************************************** This file outlines the algorithm for Microservice Manager responsible for paymentservice microservice **********************************************
...
def Monitor ():
    ...
    while Available_Replicas != Operational_Replicas:               # To accomodate pod initialization time
        ...
        Available_Replicas = "kubectl get deployment paymentservice -o=jsonpath='{.status.availableReplicas}'"
        ...
        Replicas_CPU_usage = 'kubectl top pods -l app=paymentservice'                                     # Monitoring CPU Usage per replica
    ...
    Desired_Replicas = "kubectl get deployment paymentservice -o=jsonpath='{.spec.replicas}'"
    ...
    cpu_request = "kubectl get deployment paymentservice -o=jsonpath='{.spec.template.spec.containers[0].resources.requests.cpu}'"
...
def paymentservice(Test_Time):
    ...
    paymentservice_data = [microservice_name, scaling_action, desired_replica, current_replicas, cpu_request, max_replica]

    
    #********************************************************************** Storing Data for each iteration in the Knowledge Base ********************************************************************************

    workbook = load_workbook('./Knowledge Base/paymentservice.xlsx')
    ...
    workbook.save('./Knowledge Base/paymentservice.xlsx')



    return paymentservice_data
